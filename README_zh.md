...
### 智能 TUI
- **工具调用动画** — 运行中的工具显示字符循环跑马灯动画，带灰度渐变和省略号特效
- **智能结果渲染** — 自动检测内容类型（JSON/diff/markdown/纯文本）并以相应样式渲染
- **Unified Diff 高亮** — 文件编辑返回 unified diff，ANSI 彩色渲染（绿/红增删，青色 hunk 头）
- **内联授权确认** — WorkspaceGuard 在 TUI 内直接弹窗确认危险操作，支持 Allow/Deny/Allow-All
- **会话级"全部允许"** — 对每个工具可一次性授权整个会话
- **类 Claude Code 美学** — 极简设计，彩色编码的 Agent 消息、工具状态图标、行号代码输出
...
- **结构化 LLM 日志** — 所有 LLM 输入输出记录到 `~/.codeactor/logs/llm-{date}.log`，JSON 格式化
...
### 开发者体验
- **内嵌二进制分发** — `codeactor-codebase` 二进制通过 `embed.FS` 嵌入 Go 二进制，自动提取到 `~/.codeactor/bin/`
- **Node.js CLI 客户端** — 全功能命令行工具（`run`、`chat`、`status`、`cancel`、`history`、`memory`、`load`），WebSocket 流式传输
- **对话持久化** — 任务记忆保存至 `~/.codeactor/tasks/{taskID}.json`，可跨会话恢复
- **i18n 国际化** — 通过 `LanguageManager` 提供中英文界面文本
